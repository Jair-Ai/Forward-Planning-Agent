...
class PlanningGraph:
    ...
    def h_setlevel(self):
        ...
        def check_all_goals(layer):
            for goal in self.goal:
                if goal not in layer:
                    return False
            return True
        
        def check_mutex(layer):
            for goal_one, goal_two in combinations(self.goal, 2):
                if layer.is_mutex(goal_one, goal_two):
                    return False
            return True
            
        while not self._is_leveled:
            actual_layer = self.literal_layers[-1]
            if check_all_goals(actual_layer) and check_mutex(actual_layer):
                return best_level
            self._extend()
            best_level+=1
        return -1
